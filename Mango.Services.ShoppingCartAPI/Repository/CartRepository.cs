...
namespace Mango.Services.ShoppingCartAPI.Repository
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<bool> ClearCart(string UserId)
        {
            ...
            {
                _db.CartDetails.RemoveRange(_db.CartDetails.Where(u =>
                    u.CartHeaderId == cartHeaderFromDb.CartHeaderId)); // remove different products in cart
                _db.CartHeaders.Remove(cartHeaderFromDb); // remove only one header of the cart
        ...
        public async Task<CartDto> CreateUpdateCart(CartDto cartDto)
        {
            ...
            if (cartHeaderFromDb == null)
            {
                //create header and details and link them in db
                _db.CartHeaders.Add(cart.CartHeader); // cart header id is generated here
                await _db.SaveChangesAsync();
                cart.CartDetails.FirstOrDefault().CartHeaderId = cart.CartHeader.CartHeaderId; // link relation id
        ...
        public async Task<bool> RemoveFromCart(int cartDetailsId)
        {
            try
            {
                CartDetails cartDetails = await _db.CartDetails.FirstOrDefaultAsync(u =>
                    u.CartDetailsId == cartDetailsId);

                int totalCountOfCartItems = _db.CartDetails.Where(u =>
                    u.CartHeaderId == cartDetails.CartHeaderId).Count();

                _db.CartDetails.Remove(cartDetails);
                if (totalCountOfCartItems == 1) // remove header is it's the last product
                {
                    var cartHeaderToremove = await _db.CartHeaders.FirstOrDefaultAsync(u =>
